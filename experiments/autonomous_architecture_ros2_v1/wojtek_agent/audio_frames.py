"""Audio-frame primitives shared by the demo app and the ROS voice nodes.

The browser sends 100 ms frames of mono PCM16 at 24 kHz (the AudioWorklet
capture path).  This module turns that stream into utterances and provides the
framing/resampling helpers around it.  It is deliberately dependency-light
(numpy only) and knows nothing about asyncio, ROS or any model, so both
`wojtek_agent.voice` (demo app) and `wojtek_voice.transport` (ROS nodes) build
on ONE implementation instead of hand-synced copies.

Segmentation is energy-based by default: no extra dependency, microseconds per
frame, and the failure mode that matters (cutting a sentence in half) is
governed by the silence timeout, not by frame-level precision.  Pass a `vad`
callable to swap in silero/pyannote.
"""
...
__all__ = [
    "FRAME_MS", "MAX_UTTERANCE_S", "MIN_UTTERANCE_S", "PREROLL_S", "SAMPLE_RATE",
    "SILENCE_END_S", "SPEECH_RMS", "Utterance", "VoiceSegmenter", "frame_rms",
    "pcm_frames", "resample_linear",
]
