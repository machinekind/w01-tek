...
/*=========================================================================
    ...
    -----------------------------------------------------------------------*/
    /** 7-bit I2C address for this sensor. Datasheet default is 0x68 (ADO/SDO
        pin low), but this specific board pulls ADO high -- confirmed 0x69
        via an I2C bus scan on the actual hardware. */
    #define BMX160_ADDRESS           (0x69)
    /** Device ID for this sensor (used as sanity check during init).
        0xD8 (this library's original value) is BMX160's chip ID; a bare
        BMI160 reports 0xD1 -- confirmed by directly reading register 0x00
        on the actual hardware. They are not the same die/ID despite
        sharing the accel/gyro register map. */
    #define BMX160_ID                (0xD1)
...
/*=========================================================================
    ...
    */
    typedef enum
    {
		...
		SENSOERTIME_1	= 0x19, 
		SENSOERTIME_2	= 0x1A, 
		BMX160_STATUS		= 0x1B,  // renamed from STATUS: collides with a deprecated
										// typedef in ESP-IDF's ets_sys.h on arduino-esp32 3.x
		INT_STATUS_0	= 0x1C, 
		INT_STATUS_1	= 0x1D, 
		INT_STATUS_2	= 0x1E, 
		INT_STATUS_3	= 0x1F,
