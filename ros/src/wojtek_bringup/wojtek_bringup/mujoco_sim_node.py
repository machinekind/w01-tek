...
class MujocoSimNode(Node):
    ...
    def _tick(self):
        ...
        while self.data.time < elapsed and steps < 200:
            mujoco.mj_step(self.model, self.data)
            steps += 1
        # A CPU-bound sim silently plays in slow motion (and the wall-clock
        # policy node then over-steps its gait clock per sim second), which
        # reads as "the real robot is way faster than sim". Make it loud.
        lag = elapsed - self.data.time
        if lag > 0.5:
            self.get_logger().warning(
                f"sim {lag:.1f}s behind wall clock -- physics is running "
                "below real time; do not compare motion speed against this",
