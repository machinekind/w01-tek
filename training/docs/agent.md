...
`wojtek_rl/agent/`; the design notes below explain the pieces that are not
...
Model and option survey behind these choices: [polish-voice.md](polish-voice.md).
...
`wojtek_rl/agent/asr_server.py` serves `POST /transcribe` (WAV in, text out),
...
`scripts/vast_stack.sh` provisions both on one vast.ai box —
`plan | check | deploy | serve | tunnel | status | logs | stop | env`. It
never creates or destroys an instance: `plan` prints the `vastai search` /
`create` commands and the sizing, you run those. Sizing in short: FutureNav
wants ~10 GB (its own deploy notes ask for 24 GB to be comfortable) and vLLM
~8 GB at `--gpu-memory-utilization 0.45`, so one ≥40 GB card is the easy path
and two 24 GB cards (`FUTURENAV_GPU=0 VLLM_GPU=1`) the cheap one. The script
carries the traps that cost time before: the cu124 torch pin (an unpinned
wheel silently falls back to CPU on vast's older drivers), `ninja` on PATH,
...
(`wojtek_rl/agent/trace.py`) answers "what did it do ten minutes ago", and
