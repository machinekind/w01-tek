...
| `terrain_blind_v5` | joystick | The terrain family rebased onto the flat tau_ff operating point (`flat_tff_rnd_v1`): soft kp20/kd0.5 plant, ±3 Nm feed-forward head, clock-free critic, RND, simplified rewards — plus the terrain arena/curriculum, the IMU back in the actor (blind layout), the clean height scan on the critic, and `task.env.action_filter=0.5` against the standing limit cycle observed on the real robot (the filter value is exported in the schema-2 contract and mirrored by the deployment loop). A new line; checkpoints share nothing with `terrain_blind_v4_x`. |
| `terrain_blind_v5_1` | joystick | `terrain_blind_v5` plus `task.env.obs_noise.gyro_bias=0.05` rad/s, the per-episode zero-rate offset on the actor's gyro that the white gyro noise cannot represent. |
