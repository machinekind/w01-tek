...
| `terrain_blind_v3` | joystick | Run two of the terrain family: `terrain_blind_v2`'s operating point plus clean backward commands, product/relative tracking, an orientation tolerance cone, apex and landing shaping, mirrored worlds, and a flat level-0 row, at a 32 Nm torque cap. Not deployable — that cap breaks the ±9 Nm deployment contract. |
