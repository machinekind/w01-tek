...
| Chatterbox multilingual, **DGX Spark GB10 (the target)** | first piece **0.62–1.39 s** | **0.44** |

On the deployment hardware the cloned voice generates at more than twice real
time, so clause-level streaming is comfortable and cannot underrun. The Ada
numbers are kept because they are what the demo ran on in August — and
because they are why the demo felt slow.
