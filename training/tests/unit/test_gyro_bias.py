...
default and the v5_1 preset wiring. The reset-time draw itself needs a
...
def test_v5_1_preset_is_v5_plus_gyro_bias():
    v5 = yaml.safe_load(
        (EXPERIMENT_DIR / "terrain_blind_v5.yaml").read_text()
    )
    v51 = yaml.safe_load(
        (EXPERIMENT_DIR / "terrain_blind_v5_1.yaml").read_text()
    )
    assert v51["task"]["env"]["obs_noise"] == {"gyro_bias": 0.05}
    assert v51["run_name"] == "wojtek_terrain_blind_v5_1"
    # Everything else is v5's operating point, unchanged.
    del v51["task"]["env"]["obs_noise"]
    v5.pop("run_name"), v51.pop("run_name")
    assert v51 == v5
