...
t0(){
  ...
  for f in \
    ...
    ros/src/fbb_policy/config/policy.npz \
    ros/src/fbb_policy/config/policy_meta.json \
...
t3(){
  ...
  if docker build -f ros/docker/Dockerfile -t shrek-verify:ros ros >/tmp/verify_docker.$$ 2>&1; then
    pass "colcon build (four_bar_bot_description, md80_hw, bmx160, bmi160, fbb_policy, piesek_bringup)"
  else
